...
const API_BASE_URL = process.env.NEXT_PUBLIC_API_BASE_URL + "/api/users";
class PersonalColorAnalyzer {
  ...
  static async updatePersonalColor(newColor) {
    ...
    // body를 한 번만 읽도록 처리
    let data;
    const text = await response.text(); // 먼저 text로 읽기
    try {
      data = JSON.parse(text); // JSON이면 파싱
    } catch {
      data = text; // JSON 아니면 문자열 그대로
    }
