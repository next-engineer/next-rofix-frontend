...
/** 백엔드 API용 사용자 ID 가져오기 (숫자 형태로 변환) */
export function getUserId() {
  const user = getUser();
  if (!user || !user.id) return null;

  // ID를 숫자로 변환 (백엔드에서 userId를 숫자로 받는 것으로 보임)
  const numericId = parseInt(user.id.replace(/^u-/, '').replace(/-.*$/, ''), 10);
  return isNaN(numericId) ? Date.now() : numericId; // fallback으로 timestamp 사용
}

/** 사용자 ID를 숫자로 설정/업데이트 */
export function ensureUserHasNumericId() {
  let user = getUser();
  ...
  if (!user.userId) {
    const numericId = parseInt(user.id.replace(/^u-/, '').replace(/-.*$/, ''), 10);
    user.userId = isNaN(numericId) ? Date.now() : numericId;
    setUser(user);
  }
