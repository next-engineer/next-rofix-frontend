...
class ClothesManager {
  ...
  */
  ...
  static async getWardrobeByIdFromApi() {
    const response = await fetch(`${API_BASE_URL}/api/clothes/user`, {
      method: "GET",
      credentials: "include",
    });
  ...
  static async deleteWardrobeFromApi(clothingId) {
    const response = await fetch(`${API_BASE_URL}/api/clothes/${clothingId}`, {
      method: "DELETE",
    });
  ...
  static async createClothing(clothingData, file) {
    ...
    if (!response.ok) {
      ...
      // 사용자에게 명확한 에러 메시지 전달
      throw new Error(`옷 생성에 실패했습니다: ${errorText || response.statusText}`);
    }
